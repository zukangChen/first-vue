<template>
  <div class="wrapper">
    ...
    <div class="songdetail" v-show="!full" >
      ...
      <div class="arrow" @click="gabacktwo"  v-else-if="type=='songSquare'">
        <img src="@/assets/arrow.png" alt />
        <span>歌单</span>
      </div>
      <div class="arrow" @click="gabackthree"  v-else-if="type=='user'">
        <img src="@/assets/arrow.png" alt />
        <span>歌单</span>
      </div>
...
export default {
  ...
  created() {
    ...
    api.SongList(params).then(res => {
      if (res.code === 200) {
        console.log(res);
        this.songdetail = { ...this.songdetail, ...res.playlist };
        this.songs = res.playlist.tracks
        // console.log(this.songdetail.tracks);
        this.creator = this.songdetail.creator;
      }
    });
  },
  methods: {
    ...
    //返回用户歌单(本来是应该返回用户歌单的，但是用户id的cook值没有设置时间,每次进入都要重新登录,所以暂时返回主页面)
    gabackthree(){
